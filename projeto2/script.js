let participantes = [
    {
      nome: "Alysson Martins",
      email: "alyss123@gmail.com",
  ...
  const criarNovoParticipante = (participante) => {
    ...
    const editarExcluir = `
      <button
        data-email="${participante.email}"
        onclick="editarParticipante(event)"
      >
        Editar
      </button>
      <button
        data-email="${participante.email}"
        onclick="excluirParticipante(event)"
      >
        Excluir
      </button>
    `;
  ...
  const editarParticipante = (event) => {
    const email = event.target.dataset.email;
    const participante = participantes.find((p) => p.email === email);
    const novoNome = prompt('Digite o novo nome:', participante.nome);
    if (novoNome !== null) {
  ...
  const excluirParticipante = (event) => {
    const email = event.target.dataset.email;
    const confirmacao = confirm('Tem certeza que deseja excluir este participante?');
    if (confirmacao) {
